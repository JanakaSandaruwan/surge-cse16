...
<?php
  ...
  if(isset($_GET['id'])){
    ...
    $query1="SELECT * FROM mark1 WHERE id={$user_id} LIMIT 1";
    ...
    if($result_set1){
      if(mysqli_num_rows($result_set1)==1){
        ...
        $table1 .= "<tr>";
        $table1 .= "<td>Module1</td>";
        $table1 .= "<td>{$result1['module1']}</td>";
        $table1 .= "</tr>";
        $table1 .= "<tr>";
        $table1 .= "<td>Module2</td>";
        $table1 .= "<td>{$result1['module2']}</td>";
        $table1 .= "</tr>";
        $table1 .= "<tr>";
        $table1 .= "<td>Module3</td>";
        $table1 .= "<td>{$result1['module3']}</td>";
        $table1 .= "</tr>";
        $table1 .= "<tr>";
        $table1 .= "<td>Module4</td>";
        $table1 .= "<td>{$result1['module4']}</td>";
        $table1 .= "</tr>";
    ...
    $query2="SELECT * FROM mark2 WHERE id={$user_id} LIMIT 1";
    ...
    if($result_set2){
      if(mysqli_num_rows($result_set2)==1){
        ...
        $table2 .= "<tr>";
        $table2 .= "<td>Module1</td>";
        $table2 .= "<td>{$result2['module1']}</td>";
        $table2 .= "</tr>";
        $table2 .= "<tr>";
        $table2 .= "<td>Module2</td>";
        $table2 .= "<td>{$result2['module2']}</td>";
        $table2 .= "</tr>";
        $table2 .= "<tr>";
        $table2 .= "<td>Module3</td>";
        $table2 .= "<td>{$result2['module3']}</td>";
        $table2 .= "</tr>";
        $table2 .= "<tr>";
        $table2 .= "<td>Module4</td>";
        $table2 .= "<td>{$result2['module4']}</td>";
        $table2 .= "</tr>";
      }
    }
...
<?php 
  ...
  if(isset($_POST['submit'])){//check whether user submit
    ...
    if(empty($errors)){
      ...
      $query="INSERT INTO acalist (id) VALUES({$user_id})";
...
<html>
<head>
  <link rel="stylesheet" type="text/css" href="css/std_progress.css">
          ...
          <div id="main-wrapper" class="col-md-11 pull-right">
              <div id="main" style="padding-left: 50px;">
                <div class="page-header">
                  <h3 style="font-weight: bold; font-family:'Courier New'; padding-top: 10px"><?php echo "Index Number :{$result1['id']} Name :{$result1['std_name']}"?></h3>
                </div>
                <div class="table1">
                  <h4>
                    Result of current semester
                  </h4>
                ...
                <!--div class="btn" >
                  <form method="post" action="std_progress.php">
                    <input type="checkbox" name="btn"  >  Add to the acodemic consession list
                  </form>
                </div>

                <div class="comment">
                  <p >Add any comment on student progress</p><br>
                  <form method="post" action="std_progress.php">
                    <textarea name="area" id="area" rows="3" cols="180"></textarea><br>
                  </form>
                </div-->
                <div class="btnsubmit">

                  <button type="submit" name="submit" method="post" action="std_progress.php">Add to the acodemic consession list</button>
